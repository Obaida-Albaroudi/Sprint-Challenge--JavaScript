...
//The resulting contact information should have a space between the first name and the email information like this: Name email@example.com Log the result of your new array. 
...
zooAnimals.forEach((animals) => {
  return animalNames.push(`Name: ${animals.animal_name}, Scientific: ${animals.scientific_name}`);
});
...
zooAnimals.map((animals) => {
  return lowerCase.push(animals.animal_name.toLowerCase());
});
...
const populationTotal = zooAnimals.reduce((start, animals) => {
  return start + animals.population;
}, 0)
